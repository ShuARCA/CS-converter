...
export const ERROR_MESSAGES = {
  ...
  IACHARA_REDIRECT: 'いあきゃらのURLには直接対応していません。いあきゃらのココフォリア駒出力機能でJSONを出力し、その内容を入力欄に貼り付けてください。',
  ...
  TIMEOUT: 'タイムアウトしました。時間をおいて再度お試しください。',
  CHARAENO_FETCH_FAILED: 'キャラエノからの自動取得に失敗しました。\nキャラエノ画面の「ココフォリア出力」を押してコピーし、その内容を入力欄に直接貼り付けてください。',
...
export const CORS_PROXIES = [
  ...
  {
    name: 'sirjosh_workers',
    buildUrl: (url) => `https://cors-get-proxy.sirjosh.workers.dev/?url=${encodeURIComponent(url)}`,
    extractBody: async (res) => {
      if (res.status === 404) throw new AppError('HTTP_NOT_FOUND');
      if (!res.ok) throw new AppError('HTTP_ERROR', `HTTP ${res.status}`);
      return await res.text();
