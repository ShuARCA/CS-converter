...
// SVG アイコン定義（アクセシビリティ対応: 装飾用SVGに aria-hidden/focusable を付与）
const SVG_ICONS = {
  load: `<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="slot-btn-icon" aria-hidden="true" focusable="false"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>`,
  save: `<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="slot-btn-icon" aria-hidden="true" focusable="false"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg>`,
  delete: `<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="slot-btn-icon" aria-hidden="true" focusable="false"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg>`,
  edit: `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="slot-btn-icon" aria-hidden="true" focusable="false"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>`,
  check: `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><polyline points="20 6 9 17 4 12"/></svg>`,
  cancel: `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>`,
};

/**
 * モーダル内のキーボードフォーカストラップ (WCAG 2.4.3)
    ...
    if (slot) {
      ...
      slotCard.innerHTML = `
        <div class="preset-slot-info">
          <div class="preset-slot-header-row">
            <div class="preset-slot-name-wrapper">
              ...
              <button type="button" class="preset-name-edit-btn" data-index="${index}" title="名称を変更" aria-label="スロット${index + 1} (${escapeHtml(slot.name)}) の名称を変更">
                ${SVG_ICONS.edit}
              </button>
            </div>
          </div>
        ...
        <div class="preset-slot-actions">
          <button type="button" class="preset-action-btn preset-btn--load" data-index="${index}" title="読込：このプリセットを適用" aria-label="スロット${index + 1} (${escapeHtml(slot.name)}) を読込">
            ${SVG_ICONS.load}
          </button>
          <button type="button" class="preset-action-btn preset-btn--save" data-index="${index}" title="上書き：現在の設定で保存" aria-label="スロット${index + 1} (${escapeHtml(slot.name)}) を現在の設定で上書き保存">
            ${SVG_ICONS.save}
          </button>
          <button type="button" class="preset-action-btn preset-btn--delete" data-index="${index}" title="削除：このプリセットを削除" aria-label="スロット${index + 1} (${escapeHtml(slot.name)}) を削除">
            ${SVG_ICONS.delete}
          </button>
        </div>
      `;
      ...
      slotCard.innerHTML = `
        ...
        <div class="preset-slot-actions">
          <button type="button" class="preset-action-btn preset-btn--load" disabled title="保存されたデータがありません" aria-label="スロット${index + 1} は空きスロットです">
            ${SVG_ICONS.load}
          </button>
          <button type="button" class="preset-action-btn preset-btn--save" data-index="${index}" title="保存：現在の設定を保存" aria-label="スロット${index + 1} に現在の設定を保存">
            ${SVG_ICONS.save}
          </button>
          <button type="button" class="preset-action-btn preset-btn--delete" disabled title="保存されたデータがありません" aria-label="スロット${index + 1} は空きスロットです">
            ${SVG_ICONS.delete}
          </button>
        </div>
      `;
...
function handleStartRename(slotCard, slotIndex, currentName) {
  ...
  nameWrapper.innerHTML = `
    <div class="preset-rename-form">
      <input type="text" class="preset-rename-input editable-field" value="${escapeHtml(currentName)}" maxlength="30" placeholder="プリセット名称" aria-label="新しいプリセット名称">
      <div class="preset-rename-actions">
        <button type="button" class="preset-rename-btn preset-rename-btn--confirm" title="確定" aria-label="名称変更を保存">
          ${SVG_ICONS.check}
        </button>
        <button type="button" class="preset-rename-btn preset-rename-btn--cancel" title="キャンセル" aria-label="名称変更をキャンセル">
          ${SVG_ICONS.cancel}
        </button>
      </div>
    </div>
