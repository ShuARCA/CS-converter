 ...
 *
 * 高機能カラーピッカーコンポーネント
 * SV（彩度・明度）パネル、Hueバー、Alphaバー（オプション）、HEX入力、RGB/HSL数値入力、EyeDropperに対応。
 */
...
export const ColorMath = (() => {
  ...
  const rgbaToHex = ({ r, g, b, a }, noAlpha = false) =>
    '#' + compHex(r) + compHex(g) + compHex(b) + (noAlpha ? '' : alphaHex(a));

  /**
   * 色文字列を解析して {r, g, b, a} を返す
   * 対応形式: hex (#RGB, #RRGGBB, #RRGGBBAA), rgb, rgba, transparent, 名前付きカラー
   */
...
export class ColorPicker {
  /**
   * @param {HTMLElement} container - ピッカーを表示するコンテナ要素
   * @param {Object} options - オプション
   * @param {string} [options.color] - 初期色 (#RRGGBB または #RRGGBBAA)
   * @param {boolean} [options.hasAlpha=false] - アルファチャンネルを有効にするか (本ツールでは false)
   * @param {Function} [options.onChange] - 色変更時のコールバック (hexString) => {}
   */
  constructor(container, options = {}) {
    this.container = container;
    this.onChange = options.onChange || (() => { });
    this.hasAlpha = options.hasAlpha === true; // デフォルトは false

    this.color = { r: 164, g: 194, b: 244, a: 1 }; // デフォルト色 (#A4C2F4)
  ...
  setColorStr(colorStr) {
    const p = ColorMath.parseColor(colorStr);
    if (p) {
      if (!this.hasAlpha) {
        p.a = 1; // アルファ無効時は強制的に1
      }
      this._updateState(p, false); // 通知なしで内部状態更新
      this._render();
    }
  }
  ...
  /**
   * 現在の色をHEX文字列で取得
   * @returns {string} #RRGGBB (または hasAlpha=true 時は #RRGGBBAA)
   */
  getColorStr() {
  ...
  _createDOM() {
    ...
    el.innerHTML = `
      <div class="cp-sv" id="cp-sv" aria-label="彩度・明度パネル" tabindex="0">
      ...
      <div class="cp-controls">
        <div class="cp-top">
          ...
          <div class="cp-hex">
            <input class="cp-hex-input" id="cp-hex-input" placeholder="${this.hasAlpha ? '#FFFFFF00' : '#FFFFFF'}" spellcheck="false">
          </div>
          <button type="button" class="cp-btn" id="cp-eye-btn" title="スポイトで画面から色を取得">
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 -960 960 960" fill="currentColor"><path d="M120-120v-190l358-358-58-56 58-56 76 76 124-124q5-5 12.5-8t15.5-3q8 0 15 3t13 8l94 94q5 6 8 13t3 15q0 8-3 15.5t-8 12.5L705-555l76 78-57 57-56-58-358 358H120Zm80-80h78l332-334-76-76-334 332v78Zm447-410 96-96-37-37-96 96 37 37Zm0 0-37-37 37 37Z"/></svg>
          </button>
        </div>
        <div>
        ...
        <div class="cp-compact">
          <div class="cp-group" id="cp-inputs">
            <div class="cp-box"><div class="cp-value"><input id="cp-i1" type="number" min="0" max="255"></div><div id="cp-l1" class="cp-label-small">R</div></div>
            <div class="cp-box"><div class="cp-value"><input id="cp-i2" type="number" min="0" max="255"></div><div id="cp-l2" class="cp-label-small">G</div></div>
            <div class="cp-box"><div class="cp-value"><input id="cp-i3" type="number" min="0" max="255"></div><div id="cp-l3" class="cp-label-small">B</div></div>
          ...
          <button type="button" class="cp-toggle" id="cp-mode-toggle" title="RGB / HSL 表示切替">
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 -960 960 960" fill="currentColor"><path d="M320-440v-287L217-624l-57-56 200-200 200 200-57 56-103-103v287h-80ZM600-80 400-280l57-56 103 103v-287h80v287l103-103 57 56L600-80Z"/></svg>
          </button>
        </div>
      </div>
    `;
  ...
  _bindEvents() {
    ...
    this.refs = {
      ...
      cursor: q('#cp-cursor'),
      hueBar: q('#cp-hue-bar'),
      hueHandle: q('#cp-hue-handle'),
      alphaBar: q('#cp-alpha-bar'),
      alphaHandle: q('#cp-alpha-handle'),
      alphaGrad: q('#cp-alpha-grad'),
      alphaRange: q('#cp-alpha-range'),
      preview: q('#cp-preview-fill'),
      hexInput: q('#cp-hex-input'),
      eyeBtn: q('#cp-eye-btn'),
      i1: q('#cp-i1'),
      i2: q('#cp-i2'),
      i3: q('#cp-i3'),
    ...
    refs.hueBar.style.background =
      'linear-gradient(to right,#ff0000 0%,#ffff00 17%,#00ff00 33%,#00ffff 50%,#0000ff 67%,#ff00ff 83%,#ff0000 100%)';

    // ドラッグヘルパー
    const wire = (el, start) => {
      const rel = (e, targetEl) => {
      ...
      el.addEventListener('pointerdown', (ev) => {
        ...
        const move = (e) => {
          e.preventDefault();
          if (h.onMove) h.onMove(rel(e, el), e);
        };
        const up = (e) => {
          try {
    ...
    wire(refs.sv, (ev, pos) => {
      ...
      return {
        onMove: (p) => {
          this._updateHsv({ s: p.x * 100, v: (1 - p.y) * 100 });
          this._render();
        },
      };
    });
    ...
    if (this.hasAlpha) {
      wire(refs.alphaBar, (ev, pos) => {
        ...
        return {
          onMove: (p) => {
            this._updateState({ a: Math.min(1, Math.max(0, p.x)) });
            this._render();
          },
        };
      });
    ...
    refs.hexInput.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') refs.hexInput.blur();
    });

    // EyeDropper
    refs.eyeBtn.addEventListener('click', async () => {
      if (!('EyeDropper' in window)) {
        alert('ご使用のブラウザはスポイト機能（EyeDropper API）に対応していません。');
        return;
      }
      try {
        // @ts-ignore
        ...
        if (res && res.sRGBHex) {
          ...
          if (p) {
            p.a = this.hasAlpha ? this.color.a : 1;
            this._updateState(p);
            this._render();
          }
        }
      } catch (e) {
        // ユーザーによるキャンセル等は正常動作として無視
      }
    });
    ...
    const clampInt = (v, min, max) =>
      Math.min(max, Math.max(min, Number.isFinite(Number(v)) ? Math.round(Number(v)) : 0));

    inputs.forEach((el, idx) => {
      if (!this.hasAlpha && idx === 3) return;
      ...
      el.addEventListener('change', () => {
        ...
        if (this.currentMode === 'rgba') {
          ...
          if (idx === 0) nh = ((clampInt(val, 0, 360) % 360) + 360) % 360;
          else nh = curHsl.h;
          if (idx === 1) ns = clampInt(val, 0, 100);
          else ns = curHsl.s;
          if (idx === 2) nl = clampInt(val, 0, 100);
          else nl = curHsl.l;
  ...
  _render() {
    ...
    if (currentMode === 'rgba') {
      ...
      refs.l1.textContent = 'R';
      refs.l2.textContent = 'G';
      refs.l3.textContent = 'B';
      ...
      refs.l1.textContent = 'H';
      refs.l2.textContent = 'S';
      refs.l3.textContent = 'L';
      if (hasAlpha) refs.l4.textContent = 'A';
    }
  }
